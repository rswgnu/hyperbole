  ...
  (let* ((enable-recursive-minibuffers t)
	...
	(progn (make-directory default-directory t)
		 ...
		 (let ((create-gbut (format "abcd RET link-to-file RET %s RET y C-x C-s" linked-file))
		   ...
		   (hy-test-helpers:ert-simulate-keys (kbd create-gbut)
		   ...
		   (hy-test-helpers:ert-simulate-keys (kbd edit-gbut)
...
(ert-deftest hui-ebut-create-link-to-www-url ()
  ...
  (let ((file (make-temp-file "hypb_" nil ".txt")))
    (unwind-protect
        (find-file file)
        (hy-test-helpers:ert-simulate-keys "label\rwww-url\rwww.hypb.org\r"
          (hui:ebut-create)
  ...
  (let ((file (make-temp-file "hypb_" nil ".txt")))
    (unwind-protect
        ...
        (hy-test-helpers:ert-simulate-keys "label\rwww-url\rwww.hypb.org\r"
          (hui:ebut-create)
          (hy-test-helpers-verify-hattr-at-p :actype 'actypes::www-url :args '("www.hypb.org") :loc file :lbl-key "label"))
        (hy-test-helpers:ert-simulate-keys "\r\r\r\r"
          (hui:ebut-edit "label")
          (hy-test-helpers-verify-hattr-at-p :actype 'actypes::www-url :args '("www.hypb.org") :loc file :lbl-key "label")
  ...
  (let ((file (make-temp-file "hypb" nil ".txt")))
    (unwind-protect
        (progn
          ...
          (insert "\"/tmp\"")
          (goto-char 3)
	  (hy-test-helpers:ert-simulate-keys "label\r"
  ...
  (let ((file (make-temp-file "hypb" nil ".txt")))
    (unwind-protect
        (progn
          ...
          (insert "<[label]> - rfc123")
          (goto-char 3)
	  (hy-test-helpers:ert-simulate-keys (kbd "M-DEL renamed RET")
  ...
  (let ((file (make-temp-file "hypb" nil ".txt")))
    (unwind-protect
        (progn
          ...
          (insert "<[label]> - rfc123")
          (goto-char (point-max))
	  (hy-test-helpers:ert-simulate-keys (kbd "label RET M-DEL renamed RET")
	    (hact (lambda () (call-interactively 'hui:ibut-rename))))
          (should (string= "<[renamed]> - rfc123" (buffer-string))))
      (hy-delete-file-and-buffer file))))
  ...
  (let ((filea (make-temp-file "hypb" nil ".txt"))
    ...
    (unwind-protect
        (progn
          ...
          (split-window)
          (find-file filea)
          (hy-test-helpers:ert-simulate-keys "button\r"
...
(ert-deftest hui--kill-region-delimited-text-and-yank-back ()
  ...
  (with-temp-buffer
    (let ((transient-mark-mode t)
      ...
      (fundamental-mode)
      (dolist (pos '(1 2))
        (dolist (text '("{hi}" "(hi)" "[hi]" "\"hi\""))
          (ert-info ((format "Kill thing: '%s' at pos: %s in mode: %s" text pos major-mode))
            (erase-buffer)
            (insert (format "%s\n" text))
            (goto-char pos)
            (setq last-command #'ignore)
            ...
            (should (string= (buffer-string) "\n"))
            (yank)
            (should (string= (buffer-string) (format "%s\n" text))))))

      (text-mode)
      (dolist (pos '(1 2))
        (dolist (text '("{hi}" "(hi)" "[hi]"))
          (ert-info ((format "Kill thing: '%s' at pos: %s in mode: %s" text pos major-mode))
            (erase-buffer)
            (insert (format "%s\n" text))
            (goto-char pos)
...
(ert-deftest hui-tests--selectable-thing-and-bounds ()
  "Verify `hui:selectable-thing-and-bounds'."
  (with-temp-buffer
    (let ((transient-mark-mode t)
          (mark-even-if-inactive t)
          last-command)
      ...
      (dolist (text '("{hi}" "(hi)" "[hi]"))
        (erase-buffer)
        (insert (format "%s\n" text))
        (ert-info ((format "Select thing: '%s' at pos 1" text))
          (goto-char 1)
          (should (equal `(sexp-start ,text 1 5) (hui:selectable-thing-and-bounds))))
        (ert-info ((format "Select thing: '%s' at pos 2" text))
          (goto-char 2)
          (should (equal `(symbol ,text 1 5) (hui:selectable-thing-and-bounds)))))

      (ert-info ((format "Select thing within \"\" delimiters in mode: %s" major-mode))
        (erase-buffer)
        (insert "\"hi\"\n")
        (goto-char 1)
        (should (equal '(string "\"hi\"" 1 5) (hui:selectable-thing-and-bounds)))
        (goto-char 2)
        (should (equal '(symbol "\"hi\"" 1 5) (hui:selectable-thing-and-bounds))))
      ...
      (ert-info ((format "Select thing within \"\" delimiters in mode: %s" major-mode))
        ...
        (should (equal '(string "\"hi\"" 1 5) (hui:selectable-thing-and-bounds)))
        (goto-char 2)
        (should (equal '(symbol "hi" 2 4) (hui:selectable-thing-and-bounds))))

      (ert-info ("Select thing within <> delimiters")
        (erase-buffer)
        (insert "<hi>\n")
        (goto-char 1)
        (should (equal '(sexp-start "<hi>" 1 5) (hui:selectable-thing-and-bounds)))
        (goto-char 2)
        (should (equal '(symbol "hi" 2 4) (hui:selectable-thing-and-bounds))))

      (ert-info ("Select thing within delimiters at eof fails")
        (dolist (text '("{hi}" "(hi)" "[hi]" "<hi>" "\"hi\""))
          (erase-buffer)
          (insert text)
          (goto-char 1)
          (should-not (hui:selectable-thing-and-bounds)))))))
