...
;; Last-Mod:     25-Jun-26 at 20:55:15 by Mats Lidell
;;
...
(ert-deftest hasht-tests--merge ()
  ...
  (dolist (v '(1 (1 2) (1 "2") (1 "2" [3 "4"])))
    (let* ((h (hash-make (list (cons v "k")))))
      ...
      (should (equal (if (integerp v) (list v v) v) (hash-lookup "k" (hash-merge h h))))
      (should (equal (if (integerp v) (list v v) v) (hash-lookup "k" (hash-merge (list h h)))))
...
(ert-deftest hasht-tests--merge-values ()
  ...
  (should (= 1 (hash-merge-values 1 nil)))
  (should (string= "foo" (hash-merge-values "foo" nil)))
  (should (equal '(1 1) (hash-merge-values 1 1)))
